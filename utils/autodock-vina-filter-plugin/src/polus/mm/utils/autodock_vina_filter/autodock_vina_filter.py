...
def autodock_vina_filter(input_log_path : str, input_log_paths : list[str], docking_score_cutoff : float, max_num_poses_per_ligand : int, 
                        max_num_poses_total : int, input_txt_path : str, rescore : bool):
    '''autodock_vina_filter.

    Args:
        script: 
        input_log_path: Path to the log file, Type: string, File type: output, Accepted formats: log, Example file: https://github.com/bioexcel/biobb_vs/raw/master/biobb_vs/test/reference/vina/ref_output_vina.log
        input_log_paths: Path to the log files, Type: string, File type: output, Accepted formats: log, Example file: https://github.com/bioexcel/biobb_vs/raw/master/biobb_vs/test/reference/vina/ref_output_vina.log
        docking_score_cutoff: Cutoff threshold for filtering docking scores, Type: float
        max_num_poses_per_ligand: Maximum number of poses per initial ligand, Type: int
        max_num_poses_total: Maximum number of poses total, Type: int
        input_txt_path: Experimental binding free energy data file (if any)
        rescore: Use True if autodock vina was run with --rescore
    Returns:
        None
    '''
    ...
    if input_txt_path:  # If we have experimental data
        with open(input_txt_path, mode='r', encoding='utf-8') as f:

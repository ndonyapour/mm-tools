...
def create_repository_directories(source_dir):
    """ Buid the correct directories inside mmtools-plugins.
    The directory structure must conforms to the plugin's spec :
        - dash-separated word in identifier.
        - folder hierarchy matches package namespace minus "mmtools.plugins"
        - plugin's folder name reflects the plugin package name but ends with "-plugin"
    Ex: mmtools.plugins.package1.package2.awesome_function becomes
    ...
    # remove mmtools.plugins so we only keep intermediary directories
    # Ex: mmtools.plugins.package1.package2.awesome_function creates
    # package1/package2/

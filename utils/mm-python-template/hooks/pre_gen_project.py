...
if not plugin_package.startswith("polus.mm.utils."):
    raise ValueError(
        f"plugin package must be a child of polus.mm.utils."
        + f"plugin_package must start with 'polus.mm.utils'. Got : {plugin_package}"
    )
if plugin_package.endswith("utils"):
    raise ValueError(
        f"plugin_package must not ends with _utils. Got : {plugin_package}"
    )
...
assert plugin_slug.startswith("mm-") and plugin_slug.endswith("-plugin")
